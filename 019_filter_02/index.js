...
myApp.controller('ProductController', [ '$scope' , 'ProductDataService' , function($scope, _productData) {
    ...
    $scope.init = function() {
        ...
        $scope.orderByPropertyStr = 'id';// 排序的屬性
        $scope.orderSymbol = '-';// 升冪or降冪
    };
    $scope.init();

    $scope.changeOrder = function( _orderByWhat ) {
        // console.info('_orderByWhat >>>' , _orderByWhat);
        $scope.orderByPropertyStr = _orderByWhat;
        
        switch ($scope.orderSymbol) {
            case '-':
                $scope.orderSymbol = '';
                break;
            case '':
        ...
        console.info('orderSymbol + orderByPropertyStr >>>' , $scope.orderSymbol + $scope.orderByPropertyStr);
    };
